...
public class ProductoController extends HttpServlet {
    ...
    private final String pathFiles = ManejoExcel.URL_FILES +  "producto\\";
...
//    private Set<Integer> idProductos = new HashSet();
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String estadoUnicoProducto = "Activo";
        ...
        switch (opcion) {
            case 1: // Consultar
                ...
                break;
            case 2: // Insertar
                if (Validacion.isNullStr(nombreProducto) || Validacion.isNullStr(descripcionProducto) || Validacion.isNullStr(precioProducto) || Validacion.isNullStr(stockProducto) || Validacion.isNullStr(unidadMinimaProducto) || Validacion.isNullStr(estadoUnicoProducto) || Validacion.isNullStr(idCategoria) || Validacion.isNullStr(idMarca)) {
                    ...
                    productoVo = new ProductoVO(nombreProducto, descripcionProducto, Double.parseDouble(precioProducto), Double.parseDouble(stockProducto), Double.parseDouble(unidadMinimaProducto), "", estadoUnicoProducto, Integer.parseInt(idMarca), Integer.parseInt(idCategoria));
                    this.insertProducto(request, response, productoVo, imgProducto);
                }
                break;
            case 3: // Actualizar
                if (Validacion.isNullStr(nombreProducto) || Validacion.isNullStr(descripcionProducto) || Validacion.isNullStr(precioProducto) || Validacion.isNullStr(stockProducto) || Validacion.isNullStr(unidadMinimaProducto) || Validacion.isNullStr(idCategoria) || Validacion.isNullStr(idMarca)) {
                    this.generarError(request, response, "Error al insertar el producto", "Debe proporcionar todos los datos solicitados, verifiquelos e intente nuevamente", "admin/productos.jsp");
                } else {
                    productoVo = new ProductoVO(Integer.parseInt(idProducto), nombreProducto, descripcionProducto, Double.parseDouble(precioProducto), Double.parseDouble(stockProducto), Double.parseDouble(unidadMinimaProducto), nombreImg, Integer.parseInt(idMarca), Integer.parseInt(idCategoria));
